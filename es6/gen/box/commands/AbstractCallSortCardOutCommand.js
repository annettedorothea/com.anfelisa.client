/********************************************************************************
 * generated by de.acegen
 ********************************************************************************/




import AsynchronousCommand from "../../ace/AsynchronousCommand";
import Event from "../../ace/Event";
import * as AppUtils from "../../../src/AppUtils";
import * as AppState from "../../../src/AppState";
import LoadNextCardAction from "../../../src/box/actions/LoadNextCardAction";

export default class AbstractCallSortCardOutCommand extends AsynchronousCommand {
    constructor() {
        super("box.CallSortCardOutCommand");
    }
    
    initCommandData(data) {
        data.cardId = AppState.get(
        	["rootContainer", "mainView", "queryCardView", "nextCard", "cardId"]
        )
        ;
        data.boxId = AppState.get(
        	["rootContainer", "mainView", "queryCardView", "boxId"]
        )
        ;
        data.outcomes = [];
    }

	addOkOutcome(data) {
		data.outcomes.push("ok");
	}
	
	allMandatoryValuesAreSet(data) {
		if (data.cardIds === undefined || data.cardIds === null) {
			console.warn("AbstractCallSortCardOutCommand: cardIds is mandatory but is not set", data);
			return false;
		}
		if (data.boxId === undefined || data.boxId === null) {
			console.warn("AbstractCallSortCardOutCommand: boxId is mandatory but is not set", data);
			return false;
		}
		return true;
	}

	execute(data) {
	    return new Promise((resolve, reject) => {
	    	if (this.allMandatoryValuesAreSet(data)) {
		    	let payload = {
		    		cardIds : data.cardIds,
		    		boxId : data.boxId
		    	};
				AppUtils.httpPost(
						`${AppUtils.settings.rootPath}/cards/sort-out`, 
						data.uuid, 
						true,
						 payload)
					.then(() => {
						this.handleResponse(data, resolve, reject);
					}, (error) => {
						data.error = error;
						this.handleError(data, resolve, reject);
					})
					.catch(x => reject(x));
			} else {
				resolve(data);
			}
	    });
	}
	
	publishEvents(data) {
		return new Promise((resolve) => {
			const events = [];
			const actionsToBeTriggered = [];
			if (data.outcomes.includes("ok")) {
				events.push(new Event('box.CallSortCardOutOkEvent'));
				actionsToBeTriggered.push(
					{
						action: new LoadNextCardAction(), 
						data: {
						}
					}
				);
			}
			
			this.publish(events, data).then(() => {
		  		AppState.stateUpdated();
				this.trigger(actionsToBeTriggered).then(resolve);
			});
		})
	
	}

}



/******* S.D.G. *******/



